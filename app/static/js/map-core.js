...
export async function initGoogleMap(mapId, center = { lat: 37.4979, lng: 127.0286 }, zoom = 14) {
    ...
    // Hide Google's default business/attraction pins so only OKSki Clinic/Stay/Food markers compete for attention.
...
export async function renderNearbyMarkers(pois) {
    ...
    pois.forEach(item => {
        ...
        el.setAttribute('aria-label', `OKSki ${kindLabel} pick: ${item.title || kindLabel}`);

        const marker = new AdvancedMarkerElement({
            map,
            position: { lat: parseFloat(item.lat), lng: parseFloat(item.lng) },
            title: `OKSki ${kindLabel}: ${item.title || ''}`,
            content: el,
            zIndex: 20,
        });
...
export function filterMapByClinicIds(clinicIds, opts = {}) {
    const ids = new Set(
        (clinicIds || []).map(id => String(id || '').replace(/_(en|ko)$/i, ''))
    );
    ...
    nearbyMarkers.forEach(m => {
        const near = m.itemData?.near_resorts || m.itemData?.near_clinics || [];
        const visible = near.some(id =>
            ids.has(String(id || '').replace(/_(en|ko)$/i, ''))
        );
        m.map = visible ? map : null;
    });
...
function _showInfoWindow(marker, item) {
    ...
    google.maps.event.addListenerOnce(infoWindow, 'domready', () => {
        document.getElementById('okski-info-close')?.addEventListener('click', (e) => {
            e.preventDefault();
            closeInfoWindow();
        });
...
function _nearbyInfoHtml(item) {
    ...
    actions.push(`<button type="button" class="info-box-action info-box-action--close" id="okski-info-close"><i class="fa-solid fa-xmark"></i> Close</button>`);

    return `
        <div class="info-box-content info-box-content--nearby">
            <div class="info-box-pick">
                <span class="info-box-pick-brand">OKSki</span>
            ...
            <div class="info-box-badge info-box-badge--${escapeHtml(kindKey)} info-box-badge--pick">OKSki Pick · ${escapeHtml(kind)}</div>
            <div class="info-box-title">${escapeHtml(item.title)}</div>
