#!/bin/bash
# JPFun deployment helper script.
set -euo pipefail
...
PROJECT_NAME="${PROJECT_NAME:-jpfun}"
SERVICE_URL="${SERVICE_URL:-https://jpfun.net}"
...
Modes (default: full)
  --full           Sync images + Places fetch + optimize + build
  --content-only   Rebuild items_data.json from markdown
...
Environment overrides
  PROJECT_NAME     Default: jpfun
  SERVICE_URL      Default: https://jpfun.net
...
sync_cloud_images_to_local() {
    ...
    gcloud storage rsync "$GCS_BUCKET" "$LOCAL_IMAGES" --recursive || print_info "GCS prefix empty or missing (ok on first deploy)"
    print_ok "Cloud image sync completed"
}

process_and_upload_images() {
    print_step "STEP B: Places images + upload"
    python3 script/fetch_images.py --missing || python3 script/fetch_images.py
    ...
    print_ok "Image upload completed"
}

build_data() {
    print_step "STEP C: Build JSON"
    python3 script/build_data.py
    print_ok "Data build completed"
}

git_push_changes() {
    print_step "STEP D: Commit and push changes"
    git add .
...
deploy_cloud_run() {
    print_step "STEP E: Trigger Cloud Build"
